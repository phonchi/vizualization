...
# # 10. 地震目錄統計的進階課
#
# 第 5 章你已經算過 GR 律的 b 值、畫過 Omori 衰減，也在章末聽過
...
# 2012 年觀測網升級成熟（再翻倍到 4 萬）。把長期目錄按年代切開，
...
# 就是各年代的 Mc。1970 年代約在 2.5 以上，近年已壓到 2 以下。
# **這也表示：任何跨年代的地震活動度比較，若不先固定一個共同的
# Mc（取各年代的最大值），看到的「地震變多」多半只是儀器變好。**
#
# 空間上同樣如此。台灣本島測站密集，Mc 可低到 1.5–2.0；外海
# 測站涵蓋差，Mc 高達 2.5–3.2（Chan & Wu 2013）——差 1.5 個
# 規模單位，對應事件數差約 30 倍。對全目錄套用單一 Mc，等於讓
# 外海的不完整資料污染整體統計。
#
...
def b_exact(m, mc, dm=0.1):
    ...
    sigma = np.log(10) * b ** 2 * np.sqrt(m.var(ddof=1) / (len(m) - 1))
...
dpos, dc = d[d > 0], 0.2  # 取正差，門檻 0.2
b_pos = np.log10(np.e) / (dpos[dpos >= dc].mean() - dc)

fig = go.Figure()
fig.add_trace(go.Scatter(x=cutoffs, y=b_aki, mode="lines+markers",
                         name="傳統 MLE（隨截取門檻變動）",
                         line=dict(color=ACCENT)))
fig.add_hline(y=b_pos, line_color="#1baf7a",
              annotation_text=f"b-positive = {b_pos:.2f}")
...
# 不需要知道 Mc 在哪，一次就落在真值附近。
#
# 不過近年的系統性檢驗（Tinti & Gasperini 2024）也潑了盆冷水：
# 「非取**正**差不可」其實證據不足——正差、負差、絕對差表現相當，
# 真正有效的是「把太小的差修剪掉」（trimming）這個動作。方法學
# 還在演進中，這正是統計地震學活著的證據。
#
...
fig.add_trace(go.Scatter(
    ...
    name="Mc(t) 經驗式（Helmstetter et al. 2006）"))
...
# 沒發生，是記不到。紅色虛線是南加州的經驗式 $M_c(t) = M_m - 4.5
# - 0.76\log_{10}t$；台灣也有自己的版本（Tsai et al. 2012）。
...
# %% [markdown]
# 平均值確實在 1.2 附近——Båth 沒有騙人。但看看散布：從不到 0.5
# 到超過 2.5 都有。台灣的系統性研究（Chan & Wu 2013，706 個序列）
# 給出 $\bar{\Delta}_1 = 1.20 \pm 0.73$：**標準差 0.73 意味著
...
apply_layout(fig, title="除叢前後的累積事件數（紅線＝集集、0403 花蓮）",
...
# 意義。現在地基打穩了，可以開始蓋房子。下一章登場的是現代地震
# 預報的主力引擎：ETAS。它把你在第 5 章與這一章看到的兩條經驗律
