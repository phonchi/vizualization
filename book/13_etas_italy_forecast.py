...
# 它不直接替換標準預報的參數。
#
# 這個例子的Hawkes歷史仍包含已發生事件的規模。固定 $b$ 只是讓新事件規模密度不參與這次最佳化，不是丟掉親代規模。$A_j$ 中的產能和空間尺度仍會隨每個來源的 $m_i$ 改變。
#
# simplETAS原文只估兩個振幅，但採不同固定值，並使用MPS19背景空間密度。本例採均勻背景與100 km限距，正式快取又是另一個門檻下的預報。讀者不能把三者的振幅數字直接相減，當成同一模型的估計誤差；先依附錄C的對照表核對定義。
#
# 一百公里以外的觸發項設為零。
