...
lam_A = 0.3 + 0.25 * np.sin(tt / 6) ** 2                 # 模型 A
lam_B = 0.05 + 1.6 * np.exp(-0.5 * ((tt - 60) / 4) ** 2)  # 模型 B（叢集尖峰）
lam_true = 0.7 * lam_A + 0.3 * lam_B
# 由真實率抽一份事件目錄（thinning）
cand = rng.uniform(0, 100, 3000)
keep = rng.random(3000) < np.interp(cand, tt, lam_true) / lam_true.max()
ev = cand[keep][: int(lam_true.mean() * 100 * 3)]
