...
# 參數與作業化實測（第 13 章），長期（PSHA）也有國家級模型
# （第 21 章），**中期這一段目前是空白**——而 EEPAS 的在地化
# 工作，正由台灣的研究團隊進行中。填上這一格之後，
# 台灣才有材料做下一章的事：把不同時間尺度的模型組裝成一個
# 隨時間演化的作業化系統。{doc}`下一章 <22_operational_systems>`我們就去看
