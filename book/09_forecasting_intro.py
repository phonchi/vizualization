...
# # 9. 地震可以預測嗎？從預測走向機率預報
#
# 第一部的結尾（第 8 章）留下了一個誠實但不太舒服的結論：想在單一
...
# 的前兆訊號，而是你在第 5 章已經摸過的東西：地震目錄本身。第二部
...
# 有超過一成在 60 天、100 公里內被**更大**的地震跟上（Taroni et al.
# 2023）。2016 年熊本地震是最痛的一課：M6.5 發生後，日本氣象廳
# 依慣例發布「餘震」預報，28 小時後真正的主震 M7.3 才來，死傷
# 大多發生在第二震。此後 JMA 修改了整套發布程序。
#
...
# |---|---|---|---|
# | 模型 | ETAS 家族集成 | 短期 STEP/ETAS＋中期 EEPAS＋長期 PPE 混成 | Reasenberg–Jones 貝氏／ETAS |
...
# 一個：**98%**；M6.0–6.9：**41%**；M≥7：**5%**。同一份預報裡，
# 機率隨規模跨了兩個數量級。「5% 的 M≥7」該不該讓城市停擺？
...
# 而在於它建立了一套讓錯誤的機率無所遁形的制度。下一章，我們先
# 回到一切的地基：地震目錄，以及那些比第 5 章更深的坑。
