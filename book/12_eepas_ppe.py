...
# 同樣合格、但 $T_P$ 與 $A_P$ 差異可達數十倍的辨識；而且 47 個
# 主震中有 13 個（28%）連一組都找不到。第三，物理機制至今未明。
#
# 那 Ψ 還算數嗎？算。同一份研究做了乾淨的對照實驗：把目錄的
# 發生時間隨機打散，訊號消失；把餘震拿掉，訊號仍在；用只放入
# 摩擦定律與斷層交互作用的物理模擬器（RSQSim）生成合成目錄，
# **Ψ 自己長了出來**，尺度關係與真實資料一致。結論的正確措辭是:
...
for lo, la, m in ev:
    ...
    dens += (m - 5.0 + 0.1) * (1 / (np.pi * (d_km ** 2 + r2)) + s_bg)
...
# %% [markdown]
# 東部外海與西南部亮起來——過去五十年地震密集的地方。PPE 沒有
# 任何時間資訊、沒有任何物理，卻是地震預報界最重要的一類模型，
...
#
# EEPAS 自 2004 年起被移植到加州、日本、希臘、義大利，並成為
...
#    EEPAS 勝**。更深刻的是 Janus 混合實驗：即使預報視窗縮到
...
# 迴歸，得到 $-0.56$ 與 $-0.33$，真值被**回歸稀釋**夾在中間、
...
#
# 回頭看，EEPAS 像是把第一部第 8 章的方法論功課逐條寫成了數學:
...
# （第 16 章），**中期這一段目前是空白**——而 EEPAS 的開源化
# 與在地化工作，正由台灣的研究團隊進行中。填上這一格之後，
# 台灣才有材料做下一章的事：把不同時間尺度的模型組裝成一個
# 隨時間演化的作業化系統。下一章我們就去看世界各國是怎麼
# 把模型接上社會的——從 STEP 到作業化地震預報。
