...
#
# ### SUP、SVP 與 PPE：空間不均勻不等於時間相依
#
# **空間變動 Poisson**（Spatially Variable Poisson，SVP）讓不同位置有不同長期率。
# 它可以整個測試期都不更新這張背景圖。
# Biondini 等（2023）先數格內歷史事件，再作 Gaussian 平滑。
# **Gaussian 平滑**（Gaussian smoothing）以距離衰減的常態權重分攤鄰格資訊。
# 平滑尺度控制保留多少局部差異；其原文用分段目錄互相評估。
#
# | 模型 | 空間配置 | 本書或原文的更新安排 |
# |---|---|---|
# | SUP | 同面積、同期望數 | 本站固定學習期率 |
# | SVP | 歷史計數平滑成空間變動率 | 原文作為固定時間背景，也用於 ETAS-SVP |
# | PPE | 鄰近過去大震的核疊加 | 隨新增歷史與經過時間緩慢更新 |
#
# 三者都可搭配 GR 規模比例。
# 固定空間率密度寫為 $\lambda_0(x,y,m)=h_0(x,y)s(m)$。
# SUP 的 $h_0$ 為常數，SVP 的 $h_0$ 隨位置變動。
# 兩者的式子都不必含時間變數。
# PPE 的來源及時間平均則見第 9 章。
# 本站沒有新增 SVP 的計分結果；這個對照用來閱讀原文模型表。
#
# ## 4.7 本章填入的規格欄位
#
