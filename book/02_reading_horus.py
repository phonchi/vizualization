...
# 全目錄 sigMw 的中位數是 0.2。
# 顯示兩位小數，不代表單筆規模已精確到 0.01。
# 取整後仍保留原始值與不確定度，供門檻判定與資料追查。
#
# 因此 Biondini 等人（2023）把所有規模取整到 0.1，公式是
#
# $$m_{b} = \frac{\lfloor 10\,m_{\rm raw} + 0.5 \rfloor}{10}.$$
#
# **取整規模**（binned magnitude, $m_b$）用於義大利標準預報實驗的
# 輸入／目標事件判定與規模分箱。原始 Mw 仍保留，供取整邊界與量測不確定度等診斷。
# 第二部的臺灣資料另依各章指定的規模尺度與處理方式。
...
# 大量重複值可能與定位時採用**固定深度**（fixed depth）有關。
# 資料不夠決定深度時，定位程序會先指定一個值。
# 僅靠 0 或 10 km，還不能逐筆判定是否固定，也不能認定真實震源必然淺。
# 解讀深度分布前，須回查定位品質與固定深度標記。
