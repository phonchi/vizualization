...
# 及規模條件。這份快取最後到 2025 年 11 月 30 日，因此 2025 年的柱只含前十一個月，
# 不能直接當成全年數量與前幾年比較。
#
# %% tags=["remove-input"]
...
# {doc}`b 值與規模分布 <07_gr_bvalue>`及{doc}`完整度 <08_completeness>`。
#
...
# 程式設定的門檻時估計，因此空白格也不能解讀成零風險。圖題另比較
# 120.0–121.9°E、22.0–25.2°N 方框內外的格平均；方框包含部分海域，
# 不是海岸線分類，兩個平均不能稱為陸上與海上的完整度。
#
# %% tags=["remove-input"]
DEG, DM = 0.2, 0.1
...
apply_layout(fig,
             title=f"Mc 空間場（0.2° 網格，1994 年後，深度 ≤ 30 km，"
                   f"每格 ≥ {MIN_N} 筆；指定方框內平均 {mc_land:.2f}、"
                   f"框外 {mc_sea:.2f}）",
...
# 五年一段的分組，讓各段都有足夠事件。
# 左圖涵蓋 1994–2023 的六個五年組；右圖合併 1994 年至 2025 年 11 月的資料，
# 因此兩圖的期間並不完全相同。
# 每格至少一百筆才估計，白格表示資料不足。
#
...
# 百分位按圖中經緯度格點等權排序，不是地震發生機率，也不是陸地面積百分比。
#
...
apply_layout(fig,
             title=f"只用 0403 之前的目錄（{len(pre):,} 個 ML ≥ "
                   f"{MC_PPE:.0f} 事件）：震央在所繪格點約第 {pct:.1f} 百分位",
             xaxis_title="經度", yaxis_title="緯度",
             yaxis_scaleanchor="x", hovermode="closest", height=560)
fig
...
# 所介紹的用途；{doc}`模型比較 <18_test_comparison>`另負責相對資訊的評估。
#
