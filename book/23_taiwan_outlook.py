...
#   以來（更早的部分靠歷史文獻與震度反推）、涵蓋各種規模尺度、
#   也涵蓋大量 $M_L$ 2.0 以下的微小事件——2012 年觀測網升級成熟
#   之後年偵測數約 4 萬，其中大半在 $M_L$ 2.0 以下。
# - 本書使用的公開目錄是 **1973 年起、$M_L \ge 2.0$** 的版本，
#   約 35 萬筆：起始年是儀器目錄的實質起點（TTSN 啟用），規模
#   下限是這份公開版本的收錄門檻。
#
# 差距不是資料遺失，是**取樣規則不同**。引用時務必連同基準一起
# 說，否則「台灣有 67 萬筆地震」與「我用了 35 萬筆」放在同一段
# 裡，讀者只會覺得你搞丟了 32 萬筆。先看這份目錄的全貌：
...
for ev, label in [(chichi, "集集"), (hualien, "0403 花蓮")]:
    ...
    fig.add_annotation(x=ev.time, y=7.45, text=f"{label} ML {ev.ML:.1f}",
                       ...
                       row=1, col=1)
fig.add_annotation(x="1994-06-01", y=yearly.max() * 0.92,
                   text="1994 觸發式→連續記錄（儀器）", showarrow=False,
                   font=dict(size=11), xanchor="right", row=2, col=1)
...
# 的第二次翻倍（到約 4 萬），主要發生在 $M_L$ 2.0 以下，在這份
# $M \ge 2$ 的公開目錄裡幾乎看不出來——**你看不見的改善，也是
# 一種你必須知道的改善**。
#
# 第 11 章的功課在自己的資料上一目瞭然：讀任何目錄統計圖，先問
# 哪些特徵來自地球、哪些來自儀器。台灣目錄該切的五刀，第 11.1
# 節已經列過（1973、1987.6、1991.3、1994、2012），這裡不重複，
# 只提醒一件事：**現代的台灣 ETAS 研究幾乎一律從 1994 年起算**，
# 理由就在那道階梯上。
#
# ### 三條規模轉換式，與它們對 $b$ 值的影響
#
# 台灣目錄的「規模」欄位換過三次定義，所以跨年代比較之前必須先
# 均一化。第 11.1 節擁有這三條式子，這裡只複述結果與用途：
#
# $$\begin{aligned}
...
# 立刻得到 $b = 0.862\,b'$。這條換算在 23.3 節的池上個案會派上
# 用場：文獻早年用 $M_D$ 給出該區背景 $b' \approx 1.1$，換成 $M_L$
# 表示就是 $b \approx 0.95$——**不換算就比較 $b$ 值，比較的是儀器
# 不是地殼**。
#
...
# 比對：機制解的 **Kagan 角平均差 $22.0° \pm 16.6°$**、$M_w$ 平均
# 差 $-0.08 \pm 0.10$。Kagan 角是兩個雙偶極機制之間的最小旋轉角，
# 22° 給了學生一把可用的尺——**機制解不是精確值，它自帶二十度
# 上下的不確定度**，任何建立在「機制一致」上的推論都要留餘裕。
#
# ### TSMIP 與 TGNS：波形與形變
#
# **TSMIP**（台灣強地動觀測計畫）目前有 737 個自由場強震站與 45
# 座結構物陣列，解析度已由 16 位元全面升級到 24 位元（地動振幅
# 解析能力提升約 200 倍）；三十年累積逾 28,000 個地震、22 萬筆
# 紀錄，其中 1999 年集集地震的數十筆車籠埔近斷層紀錄，讓全球的
# 近斷層強震資料一舉增加數倍。這份資料是
...
# {doc}`第 6 章 <06_gnss>`用的就是這些資料。CWBSN 本身則有超過
# 170 個測站，含 62 個平均深度 300 m 的井下站與東部海域的海底
# 地震儀站。
#
...
# 差 0.16 看起來像構造變化，其實是 $M_D \to M_L$ 的尺度換代——用
# 23.1 節的 $b = 0.862\,b'$ 檢查一遍就知道，**這個量級的落差，
# 換一次規模尺度就足以製造出來**。
#
# ### 表二：$M_c$ 與 Omori $p$
#
...
# 它至少在統計上隨主震規模增加**（完整討論見 12.3 節）。但作者
# 自己也強調，這些係數在不同目錄之間**並未**與任何構造特徵相關
# ——它是穩健的經驗關係，不是有物理解釋的定律。
#
# ### 表三：Båth 與 ETAS
#
# | 量 | 台灣值 | 但書 | 章節 |
...
# et al. (2015) 的淺層地殼分區與 81 × 201 格點。完整的八參數對照
# 表在 14.9 節，這裡只收結論：
#
# | 參數群 | 兩份估計 | 但書 | 章節 |
# |---|---|---|---|
...
# 兩份獨立估計的離散度，是我們手上唯一的不確定度來源——兩份
# 報告都沒有附標準誤。
#
# ### $M_c$ 是一張地圖：自己算一次
#
# 表二第一列說 $M_c$ 是空間場，這件事值得自己算一遍。用 0.2° × 0.2°
# 網格、1994 年後、深度 ≤ 30 km 的事件，逐格套第 11.3 節的最大
# 曲率法（MAXC + 0.2）。實作上唯一的關鍵是**別逐格去掃全目錄**：
# 正確作法是一次 `np.histogramdd` 把（經度、緯度、規模）三維直方
# 圖建好，再沿規模軸取 `argmax`——整個計算是向量化的：

# %% tags=["hide-input"]
DEG, DM = 0.2, 0.1
MIN_N = 50                       # 每格至少要有這麼多事件才估 Mc

lon_e = np.arange(119.4, 123.21, DEG)
lat_e = np.arange(21.4, 26.01, DEG)
mag_e = np.arange(1.95, 7.05, DM)
shallow = cat[(cat.time >= "1994") & (cat.depth <= 30)]

# 三維直方圖：一次掃完，不對 35 萬筆做逐格比對
H, _ = np.histogramdd((shallow.longitude.to_numpy(),
                       shallow.latitude.to_numpy(),
                       shallow.ML.to_numpy()),
                      bins=(lon_e, lat_e, mag_e))
n_cell = H.sum(axis=2)
...
mc_land = np.nanmean(mc_grid[on_land])
mc_sea = np.nanmean(mc_grid[~on_land])
...
#
# 這個限制不影響圖的主要用途，對比的**方向與量級**仍然可信：
# 本島平均與外海平均差約 0.3 個規模單位，而文獻用完整目錄得到的
# 落差可達 1.7 個單位——依 GR 律，那對應約 50 倍的事件數差異。
# 對全島套單一 $M_c$，等於讓外海的不完整資料污染整體統計。這也是
# 為什麼上面兩份 ETAS 估計都保守地取 $M_c = 3.0$：明知陸上可以
# 用到 2.0，仍取全區的上限。**寧可少用資料，不可讓不完整的資料
# 污染參數估計**。
...
# 餘震**，其中 9 個 $m \ge 6.0$；空間窗選得夠好，不到 5% 的區內
# 事件不屬於這個序列，也不到 5% 的餘震落在區外。三條定律的擬合
# 結果是 $\log_{10} N(\ge m) = 6.4 - 0.84\,m$（$2 < m < 5$），
# 即 $b = 0.84$、$a = 6.4$；Omori 部分 $p = 1.05$、
# $c_0 = 6 \times 10^{-3}$ 天。集集前六年的背景事件間隔
...
# 兩者差 0.92 個規模單位，等於在說這個序列的規模分布形狀跟 Båth
# 定律預設的形狀不一樣。
#
...
# 而不是事後在已知答案的情況下算出來的一個數字。
#
# 池上的另一個貢獻是 $p$ 值：同一份目錄、同一支程式，全期 30 天
# 得 $p = 0.92$，取前 12 天得 1.30，只取前 6 天得 **1.39**
# （完整式子 $n(t) = 27.7/(t + 0.02)^{0.92}$）。第 12.3 節解釋過
# 機制：真實序列不是單一 Omori 核，窗一拉長涵蓋到大餘震，那個
# 大餘震自己的次級序列就把尾巴撐高，單一核唯一能做的事就是把 $p$
# 調小。**報告 $p$ 值時不附上擬合時間窗，等於沒報告**。
#
# ### 2025 大埔：一條完整的作業 pipeline
#
# 2025/01/20 大埔 $M_L\,6.4$（深度 15.8 km，大埔震度 6 弱、西南部
# 5 弱）之後，台灣第一次公開發表了端到端的作業型餘震預報實測
# （Hsieh et al. 2025）。到 2/07 共觸發 162 個 $M_L \ge 3.0$ 餘震，
# 其中 8 個 $\ge 5.0$。整條流程值得逐步拆解，因為它把第 11 到 21
# 章的東西串成了一條線：
#
...
#    地殼分區、119.8–122.5°E × 21.7–25.6°N 的 81 × 201 格點。
#    訓練樣本 64,239 個 $M_L \ge 3$ 事件（第 11.3 節）。
...
# 動**。第 4 步那組數字尤其值得記住：作業化的瓶頸在預訓練，不在
# 即時運算。這解釋了為什麼全世界的作業系統架構長得都一樣——先花
# 大錢把參數訓練好、鎖起來，地震來了只更新目錄與背景率。更難得
# 的是這份報告**把預報機率與實際觀測並列發表**：
...
for name, vals, color in [("P(ML ≥ 5)", p_ge5, PALETTE[0]),
    ...
    fig.add_trace(go.Bar(x=win_label, y=vals, name=name, marker_color=color,
                  ...
                  secondary_y=False)
fig.add_trace(go.Scatter(x=win_label, y=obs_ge5, mode="lines+markers+text",
                         name="實際 ML ≥ 5 次數",
                         text=[str(n) for n in obs_ge5], textposition="top left",
                         line=dict(color=QUAKE_COLOR, width=2.5, dash="dot"),
                         marker=dict(size=11, symbol="diamond")),
              secondary_y=True)
fig.update_yaxes(title_text="至少發生一次的機率（%）", range=[0, 80],
                 secondary_y=False)
fig.update_yaxes(title_text="實際發生次數", range=[0, 10], secondary_y=True,
                 showgrid=False)
apply_layout(fig,
...
# %% [markdown]
# 這張圖必須小心讀，因為它很容易被當成「評分表」。**機率與次數是
# 兩種不同的量**：長條是「時窗內至少發生一次的機率」，紅色菱形是
# 「實際發生了幾次」，兩者不能直接對齊比高低。
#
# 可以合法說出口的觀察有兩個。第一，1 天窗的 $P(M_L \ge 5) = 30.5\%$
# 而實際發生 1 次、10 天窗 67.8% 而實際累積 8 次——次數的成長比
# 機率的成長陡，暗示模型在這個序列上可能偏保守，但**單一序列不足
# 以下這個結論**。第二，$P(M_L \ge 6)$ 從 3.4% 爬到 11.2%，而實際
# 一次都沒發生；**這不算預報失敗**，11.2% 的事件本來就有 88.8% 的
# 機會不發生。要判斷一組機率預報準不準，必須用一整批預報做一致性
# 檢定（N-test、S-test、負二項或二元似然，第 17 章）與比較檢驗
# （資訊增益，第 18 章），不能用單次結果判對錯。這件事第 14.9 節
# 講過一次，這裡值得再講一次——它是全書最容易被違反的一條原則。
#
# 序列本身還有兩個誠實面。**其一**，主震前源區沒有明顯高地震活動
# 率，即時目錄未顯示任何前驅訊號——**大埔沒有前兆**。**其二**，
# 序列呈四階段結構（主震 → 1/25 $M_L\,5.7$ → 1/30 $M_L\,5.6$ →
# 之後），每隔約 5–6 天出現一次群集；1/24 到 1/25 之間有一段
# 「在 Omori 衰減框架下不尋常的平靜」，但作者謹慎地指出**即時目錄
# 的完整度與定位精度不足以判定那是不是真平靜**。這句話點出一個
# 教科書很少提的落差，23.5 節會把它列成第三個缺口。場址端則有
# 另一個教訓：嘉義站（近）的震度超越機率曲線清楚呈現四階段結構，
# 機率峰值常對應到後續實測到的較高震度，台南站（遠）就模糊得多
# ——**距離調節了餘震對場址震度的影響**。
#
# ### 三個序列合起來說了什麼
#
# 集集告訴我們 Båth 定律的「常數」可以崩到 0.03；池上告訴我們 $p$
# 值可以在同一份資料上從 0.92 變到 1.39、$b$ 值在同一個序列裡分成
# 兩段；大埔告訴我們一整條 pipeline 可以在 7 分鐘內跑完一輪。
# 共同結論是：**定律給的是分布，不是數值**。速查表（23.2）列的是
# 分布的中心，個案（23.3）展示的是分布的寬度。把中心當成答案用，
# 遲早會在某個序列上翻車。
...
lons = np.arange(119.0, 123.51, 0.1)
lats = np.arange(21.0, 26.01, 0.1)
...
# 對「何時」完全沉默。PPE 的率密度只隨新事件加入而緩慢更新，它在
...
# 形成鮮明對比——池上有 $M_L\,6.6$ 的關山前震在前一天，而 0403
# 沒有。一套 ETAS 型的作業化餘震預報（OAF）系統在 0403 之前不會
# 發出任何警示，這不是系統失靈，而是**設計如此**：ETAS 的條件
# 強度是 $\lambda^*(t,x,y,m) = \mu(x,y) + \sum_{i:t_i<t} \kappa(m_i)
# g(t-t_i) f(x-x_i,y-y_i;m_i)$，沒有觸發事件就只剩背景項。
#
# 但主震之後就完全是另一回事。同一套系統在 0403 之後可以立刻接手，
# 而且我們知道它會表現得不錯——因為 23.3 節那條大埔 pipeline 的
# 每一個零件都是通用的：預訓練參數、$M_c$、GMM、7 分鐘一輪的算力
# 都已經到位。0403 之後台灣缺的不是技術能力。
#
# ### 中期：這一格是空白的
#
# 誠實的回答是：**沒有東西可以檢視**。台灣目前沒有作業化的中期
# （月到十年）模型，所以「0403 前一年，中期模型會說什麼」這個
# 問題連問都問不了。這不是本章的謙辭，是 23.5 節第一個缺口的
# 具體樣貌。
#
# ### 事後總能找到跡象，這正是問題所在
#
...
# 已知答案的情況下調整這三個選擇，直到訊號浮出來。這個過程在每
# 一個大地震之後都會發生，而且產生的圖片非常有說服力。第 18 章
# 給了它一個名字與一套解藥：名字是**自由度**——事後分析裡分析者
# 可以動的旋鈕，遠多於他報告出來的那幾個；解藥是**零自由度的
# 前瞻檢驗**——模型參數、預報規格、目標資料來源全部在觀測之前
# 明確定義並公開註冊，之後不准改。CSEP 整個社群存在的理由就是
# 這個。
#
...
fig.add_annotation(x=np.log10(1.5), y=3.62, text="檢驗文化（第 17–18 章）貫穿全部尺度",
...
#
# 這一段不是可有可無的。它正好對應兩類最重要的決策：災後重建的
# 中期規劃（基督城重建用的就是 50 年時變危害模型）、以及既有建物
# 補強的優先順序（紐西蘭 Kaikōura 之後的無筋磚造建築強制補強
# 政策，直接建立在中期模型上）。缺了這一格，短期預報算出來的
# 機率沒有辦法接上長期危害，只能各說各話。第 15–16 章介紹的 Ψ
# 現象與 EEPAS 正是填這一格的工具族；EEPAS 的台灣在地化工作正在
# 進行中，在成果正式發表之前，這一格在圖上只能標成紅色。
#
# ### 缺口二：預報檢驗文化尚未建立
#
# 這是四個缺口裡最不像缺口、也最關鍵的一個，因為它不是缺一個
# 模型，是缺一套**制度**。
#
# 台灣目前的預報相關研究絕大多數是**個案回溯分析**：某個序列
# 發生了，事後套模型、報參數、討論擬合品質。這類工作有價值，但
# 它無法回答「這個模型比別的模型好嗎」，因為分析者是在看過答案
# 之後才決定要看什麼。2025 大埔快報之所以難得，是因為那份預報
# **在事件發生過程中逐時窗發出**，不是事後補算的。缺的是 CSEP 式、
# 事先註冊的前瞻檢驗：模型與參數在期初鎖定並公開，目標事件的
# 定義（規模門檻、深度範圍、時空箱、要不要除叢）在期初講清楚，
# 然後等時間過去，用第 17 章的一致性檢驗與第 18 章的比較檢驗
# 評分。23.6 節整節都在講這件事該怎麼做。
#
# ### 缺口三：即時目錄與重定位目錄的落差
#
...
# 水平震央差平均 4.1 km，編號地震的偵測率約 69%。
#
# 對統計模型來說，這幾個數字的後果是連鎖的：規模系統性偏差會
# 直接搬移 $M_c$ 與 $b$ 值（第 11 章）；偵測率不足會讓早期餘震
# 缺漏，而第 14.6 節證明過，缺漏事件會讓 $\alpha$ 被低估、$c$ 被
# 高估、$p$ 被拉低。**用重定位目錄訓練、用即時目錄預報**，這個
# 錯配在每一個作業系統裡都存在，台灣也不例外。
#
# ### 缺口四：前兆研究的誠實現況
#
# 最後一個缺口，數字由業務單位自己提供。CWA 的地震前兆觀測業務
# 分中長期（震前約半年，項目為地震活動時空變化與地殼形變）與短期
# （震前數小時到 10 天，項目為電離層 TEC、地下水位、地磁、大地
# 電場）。業務回顧的原話是：真正成功的案例非常有限，尤其短期前兆
# 「**可視為成功發現前兆的比例似乎都在 2 成以下**」。
#
# 這句話不是「前兆研究沒用」，也不是「地震完全不可預測」。它的
# 正確讀法是第一部整整八章的結論：訊號太小、樣本太少、事後選擇
# 太會騙人。2016 美濃地震前，中南部山區的地震個數與能量釋放都
# 低於長期背景的第一四分位、台南歸仁—仁德的 GNSS 基線由每年縮短
# 約 1 cm 轉為「鎖住」——這些事後看起來很像前兆的訊號確實存在，
# 但沒有一個通過過前瞻檢驗。所以這個缺口不是放棄的理由，而是
# **資源配置的理由**：把力氣優先投給已經被證明有預報技巧的統計
# 叢集模型，同時把前兆研究放進與統計模型同一套檢驗框架裡（第 18
# 章的 Molchan 圖與面積技能分數就是為警報式模型設計的）。
#
# ### 四個缺口的共同點
#
...
# 實驗：在慣用的 0.1° × 0.1° 全球網格上，一個「地球上每個地方
# 機率都一樣」的**均勻模型竟然通過了 S-test**——不是模型好，是
# 檢驗沒有功效：648 萬個格子配 651 顆地震，平均一萬格才一顆，
# 空間資訊被解析度稀釋掉了。作者估計，要在那種設定下得到有力的
# 檢驗，需要 32,000 顆以上的地震，約當 300 年的記錄。
#
...
# 該檢驗在該設定下的**功效估計**（第 18.5 節），否則「沒被拒絕」
# 什麼也沒說；(b) 測試期要夠長，或明確報告單一序列的影響；
# (c) 網格設計要當成實驗設計的一部分認真對待，見下一條。
#
# ### 坑二：等寬網格會殺掉檢驗力，改用 Quadtree
#
# 上面那個問題有一半是網格造成的，而網格是我們自己選的。Khawaja
# 等人的一維合成實驗最能說明：在 $[-3,3]$ 上以標準常態抽事件、
# 用均勻模型去預報，**等寬分箱**的功效在 4 個箱時達到最高（僅約
# 0.3），之後隨箱數單調下降；**等期望率分箱**的功效卻隨箱數收斂
# 到約 0.85。固定 20 個箱時，等期望率分箱只要 15 顆事件就達到
# 功效 0.9，等寬分箱要 38 顆——同一批資料，只是換了分箱方式，
# 檢驗力就差了一倍以上。
#
# **Quadtree** 是這個想法的二維版本：整個區域當根節點，每個格子
# 要嘛不分、要嘛切成四個子格；以「每格最多容納的資料點數
# $N_{\max}$」與「最大縮放層級 $L_{\max}$」兩個參數控制，讓
# 地震多的地方細、地震少的地方粗。在多解析度網格上，S-test 只要
# 8 顆地震就能達到最大功效。預報在網格之間的轉換有理論保證：
# 聚合就是把小格的率相加，反聚合就是把大格的率均勻攤到小格，而
# Poisson 相加仍是 Poisson（率為 $\Lambda = \sum \lambda_i$），
# 所以**真模型聚合後仍會通過檢驗，假模型則會在新網格上被抓出來**。
# 對台灣這種「陸上密、外海疏」的地震分布，多解析度網格幾乎是
# 必要的——均勻網格會把大量幾乎沒有地震的海域格子塞進似然裡，
# 稀釋掉真正有資訊的那幾格。
#
# ### 坑三：台灣序列的叢集極強，Poisson 假設會低估變異
#
# 23.3 節的三個序列已經說明了問題：集集 1000 天內 42,952 個
# $m \ge 2$ 餘震、0403 序列三個月貢獻全年目標事件的大半。這種
# 分布下，「每個時空箱的事件數服從 Poisson」會嚴重低估變異——
# Poisson 的變異數等於期望值，而叢集資料的變異數遠大於期望值。
# 後果是 N-test 的信賴區間太窄，模型動不動就被拒絕，而拒絕的
# 原因是叢集不是模型錯。第 17 章給了兩條出路，兩條都適合台灣：
#
# - **負二項似然**：把 Poisson 換成有額外離散度參數的負二項分布，
#   讓變異數可以大於期望值。代價是多一個參數，而且那個參數本身
#   要從資料估——在目標事件少的台灣，這件事不便宜。
# - **二元似然（binary likelihood）**：只問「這個格子有沒有發生
#   至少一顆」，不問發生幾顆，一次把叢集造成的計數膨脹整個拿掉，
#   代價是丟棄計數資訊。Bayona 等人跨區比較時就同時報告 Poisson
#   的「每顆地震分數」與二元的「每個 active cell 分數」，因為三個
#   測試區的格子數與目標事件數差距懸殊，直接比聯合對數概似沒有
#   意義。
#
# 台灣兩者都該報。**如果一個模型只在 Poisson 似然下贏，那個勝利
# 很可能是叢集造成的假象**。
#
# ### 坑四：對齊。規模尺度、深度、除叢，一個都不能少
#
# 這一條看起來最無聊，但它是「apples-to-apples」的全部內容，而且
# 剛好把本章的第一節接回來：**23.1 的三條轉換式在這裡變成檢驗
# 設計的必要條件**。
#
# - **規模尺度**：GEAR1 用全球 CMT 的 $M_w$ 校準，台灣的目標目錄
#   是 $M_L$。Bayona 等人在義大利遇到同樣的問題，作法是把率除以
#   1.602 做尺度換算；台灣要做同一件事，用的就是
#   $M_w = 0.87 M_L + 0.23$——而且要記得 $M_L$ 在 6.5 以上飽和，
#   換算在高規模端會失真。
# - **規模門檻的外推**：GEAR1 是以 $M_w\,5.95+$ 發布的，要下放到
#   4.95+ 得用全球 $b = 1$ 外推。台灣的 $b$ 值不是 1（表一），
#   外推時用哪個 $b$ 會直接改變基準模型的率——這個選擇必須事先
#   宣告，不能等看到結果再回頭調。
...
# 長期缺乏**可重現性**（無法重新產生當初發出的預報與檢驗結果）
# 與**可複製性**（換一批資料得不到相同的技巧結論），而同儕審查
# 不足以保證這兩件事。解方是把所有模型參數、預報規格、目標資料
# 來源都在觀測之前明確定義，達成**零自由度的獨立檢驗**。落到
# 台灣的實作，最低限度是四件事：模型與參數在期初鎖定並存檔
# （附版本號）；目標事件的完整定義（規模尺度與門檻、深度範圍、
# 時空箱、除叢與否、用哪一版目錄）寫成一份公開文件；基準模型
# 至少一個（PPE 或投影到台灣的 GEAR1）；分析程式與資料打包成
# 可重現套件，讓別人一道指令重跑所有圖表。
#
# 補一句立場問題。CSEP 社群自己的說法是：**不因為模型沒通過某一項
# 檢驗就正式否決它**，而是把分位數分數當作診斷工具，指出模型與
# 資料哪裡不合、值得科學上追究。這個態度對台灣特別重要——在一個
# 目標事件這麼少的測試區，把單次檢驗結果當成生死判決，只會逼出
# 更多過度自信的宣稱。**檢驗是為了學到東西，不是為了頒獎**。
...
# 同時給平均數、範圍、機率與判讀語彙，而且這個模型不只發預報，
# 基督城重建的 50 年時變危害模型與 Kaikōura 之後的無筋磚造建築
# 強制補強政策都建立在它上面。**美國**把「什麼時候發」制度化了
# ——大震後自動觸發，不需要有人臨場決定要不要說話。
#
# 值得注意的是，這三件事**互相獨立**：台灣可以選擇公開發布但不
# 自動觸發、或自動觸發但只給民防、或要求 CSEP 檢驗但暫不公開。
# 這不是三選一的套餐。
#
# ### 台灣既有的基礎設施
#
...
# 位置與規模，並以 email 與呼叫器通報，當時是全球地震速報的
# 里程碑；2018 年花蓮地震，**震後 17 秒發布警報、20 秒**經手機與
# 電視觸達民眾，5 分鐘正式地震報告，7 分鐘鄉鎮市區尺度的細緻化
# 震度，10 分鐘內成立災害應變中心。2013 年起全國 3,500 所中小學
# 安裝地震資訊接收軟體，2016 年配合國家災防告警細胞廣播系統開發
# 地震速報廣播；台大的 P-Alert 微機電觀測網到 2023 年 8 月已建置
# 777 站。
#
...
#
# **第二塊是發布管道**。CWA 的地震事件頁、細胞廣播、與電視台的
# 蓋台插播機制，都是經過大量實戰的發布基礎設施。這在制度上意義
# 重大：三條路線裡最花錢的那一段（怎麼把訊息送到每個人手上）
# 台灣已經蓋好了。缺的是**要送什麼內容、由誰決定送不送**。
#
# ### 權威性由誰授予
#
# 這是第 22 章留下的問題，也是本章唯一無法用資料回答的問題。
# 機率預報要能發揮作用，發布者必須具備權威性——而權威性不是
# 技術指標，是社會授予的。2009 年義大利 L'Aquila 地震之後的
# 審判讓全世界的地震學家記住了一課，但這一課常常被記錯：不是
# 「說了會被告」，而是科學家與官員被批評的恰恰是**沒有把
# 「機率確實升高了」講得夠清楚、夠強**。沉默的代價與說錯話的
# 代價同樣真實。這場災難直接催生了「作業化地震預報」這個領域
# ——與其臨場即興，不如建立常態化、權威、透明的機率發布制度。
#
# 台灣要回答的具體問題有三個，每一個都不是統計學家能單獨決定的：
# 誰是法定的發布者？發布的內容是機率、情境敘事，還是行動建議？
# 當機率升高但仍然很小（第 22.3 節說過，短期預報的宿命是機率
# 增益大而絕對機率從來不高）時，發布的門檻在哪裡？第 22.3 節
# 整理過的溝通要點——用「未來一週**之內**」而非「下一週」、把
# 機率換成頻率框架、報數量範圍比報機率範圍抗扭曲、機率永遠配上
# 文字判讀——都是有實驗支持的技術，但技術解決不了「誰有資格
# 說話」。那是制度問題，而制度問題的答案不會從資料裡長出來。
...
# 事的兩面。
#
# **第一部教的是懷疑的紀律**。地下水位、地磁、GNSS、波形——四類
# 資料輪流上場，每一類都有人宣稱過它能預測地震，而每一次仔細
# 追下去，訊號都在雜訊、儀器、氣象、與事後選擇裡溶解掉。第 7 章
# 對 0403 花蓮做的四類資料對照，結論是乾淨的訊號只有波形與餘震；
# 第 8 章把方法論講白了：異常這種東西，只要你願意找，永遠找得到。
#
# **第二部教的是把懷疑制度化**。同樣的懷疑精神，如果只停在
# 「我不相信」，就只是犬儒。第 9 到 23 章走的是另一條路：先承認
# 有一種可預報性是真的（叢集——大地震之後短期內地震率確實升高
# 數十倍，這件事被反覆檢驗、被多國作業系統每天使用），再建立
# 一整套機制讓真假宣稱可以被分辨。基準模型讓「有技巧」有了對照
# 組；前瞻檢驗讓事後選擇無所遁形；資訊增益讓「比較好」變成一個
# 有單位的數字；統計功效讓「通過檢驗」這件事本身也要被檢驗。
#
# 這套機制的代價是它讓人謙虛。所以，這門學問今天能誠實說出口的
# 成果大概是這樣的：
#
# **我們能做到的**：對一個大地震之後的時空範圍，給出經得起檢驗的
# 餘震機率與場址震度機率；把這些機率接上地動預估式與工程決策；
# 用資訊增益公平比較兩個模型的優劣；在世界上好幾個地方每天運轉
# 這套流程。這不是小成就——1989 年之前，這些話沒有一句說得出口。
#
# **我們做不到的**：告訴你下一個大地震的時間與地點。判定眼前的
# $b$ 值下降是不是前震訊號。用單一序列的結果證明一個模型好或
# 不好。可靠地在中期尺度上發言（至少在台灣還不行）。以及最根本
# 的——把絕對機率從百分之幾提高到有人願意據以疏散的水準。
#
# 台灣的位置很特別：世界級的資料、世界級的預警、正在成形的短期
# 預報、一段還沒有人填上的中期空白、以及一套還沒開始運轉的檢驗
# 文化。這四件事裡，只有第一件是老天給的，其他三件都是人做出來
# 的，也就意味著都還可以再做。
#
# 那段空白，也許就留給正在讀這一頁的你。工具在前面十四章裡，
# 目錄就在 `data/cache/` 裡，缺口在 23.5 節列得清清楚楚，
# 而 23.6 節那份清單就是入場的規則。動手之前，只需要記得全書
# 說了兩遍的那句話——**一個數字算得出來，不等於它站得住腳**。
# 讓它站住腳的方法，你現在已經會了。
