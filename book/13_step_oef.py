...
fig.add_hline(y=0.5, line_dash="dash", line_color="#1baf7a",
...
# %% [markdown]
# 主震剛過時，一週內再來一個 M≥5 的機率是百分之十幾；一個月後
# 掉到百分之幾；一年後仍略高於背景。**機率增益大（相對背景高
...
# |---|---|---|---|
# | 引擎 | ETAS 家族集成，權重每週依表現更新 | 短期 STEP/ETAS＋中期 EEPAS＋長期 PPE，取最大值混成 | R–J 貝氏（國內自動）；ETAS＋萬次模擬（重大事件） |
# | 節奏 | 每日午夜＋每個 M≥3.5 後，發布一週預報 | 無固定排程，隨序列演化 | M5+ 後約 20 分鐘自動首報 |
...
# 餘震照常衰減；約 25%——發生 M7.0–7.8；約 5%——出現比主震
# 更大的地震（含板塊介面破裂與海嘯可能）。每個情境同時給
# 文字、機率與潛在衝擊，並明講「機率雖低，但比主震前高了
# 許多」。這份文件被廣泛視為機率溝通的典範。
#
# 最後一個誠實的難題：溝通研究在 Canterbury 序列六年後訪談
# 發現，理解餘震序列行為的居民在第六年的 M5.7 來襲時比較
...
# 的 PPE，各自抓住不同時間尺度的可預報性——下一章我們看
# 怎麼把三個臭皮匠湊成一個諸葛亮，以及一個著名的十年實驗
# 如何給組合模型上了殘酷的一課。
