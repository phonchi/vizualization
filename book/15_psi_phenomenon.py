...
#
# 以起點到主震的時間為 $T_P$，這段期間最大三個規模的平均為 $M_P$，包住選定事件的面積為 $A_P$，便把一張圖整理成三個尺度。以下是刻意加入活動變化的合成資料：先比較上方散佈圖的密度與高度，再看下方的 $C(t)$ 如何回應。
#
# %% tags=["remove-input"]
import numpy as np
...
#
# 下面畫出多重辨識的分布。重點是同一主震內部的變異，而不是替全部主震找一個共同的最佳數字。它會帶我們進入回歸前必須考慮的群組結構。
#
# %% tags=["remove-input"]
