...
# ## 1.5 一份能核對的預報實驗
#
# **回溯檢驗**（retrospective testing）用已存在的資料評估預報。
# 它可以使用擬合資料，也可以另留測試資料。
# **擬前瞻檢驗**（pseudo-prospective testing）是其中保留時間順序的一類。
# 模型只依指定截止前的資料製作後續預報。
# 但建模者可能早已知道後來的結果。
#
# **前瞻檢驗**（prospective testing）評估模型與建模者尚未見過的結果。
# 預報可即時產生，也可在受控環境中延後執行。
# 後者須事先固定規則，並嚴格隔離未來資料。
# 所以，前瞻與否不能只看程式執行的日期。
# 第 3 章會完整區分這些安排。
#
# 本書以 2024 年修訂版 HORUS 重演 2012–2021 年實驗。
# 這是擬前瞻教學重演，屬於回溯檢驗。
# 依事件時間截止，不代表恢復當時的目錄版本。
# 當年的速報、後來修訂的規模與位置可能不同。
# 研究區沿用已發表論文的選區。
# 該選區使用過 2021 年前的資訊。
# 所以不能把整個設計宣稱為事前凍結。
# 這個限制不妨礙學習，但會限制結論。
...
# - Mizrahi 等（2024），[預報發展、檢驗與溝通](https://doi.org/10.1029/2023RG000823)。免費綜述；圖 2 對照前瞻、回溯與擬前瞻，不要把它們和發報頻率混在一起。
