...
# 近而小（10 km 外，最大規模 6.5，活動較頻繁）與遠而大
# （40 km 外，最大規模 7.8，活動較稀）。規模服從截斷 GR
...
faults = [("近而小的斷層", 10.0, 0.10, 6.5),
          ("遠而大的斷層", 40.0, 0.02, 7.8)]
...
# 的 PGA 就是設計地動。注意兩條斷層的角色互換：低強度端由
# 近而小的斷層主導（它常動），高強度端遠而大的斷層追了上來
# （只有大地震配上運氣不好的地動殘差，才晃得出 1 g）。
#
...
# PGA > 0.2 g 的危害大半來自近處的中等地震；PGA > 1 g 則由
# 大規模地震主導。反聚合是 PSHA 與工程設計之間的翻譯器——
...
# 性，而最終那個「475 年回歸期的設計地動」以工程規範的姿態
# 出現時，所有的不確定性都藏進了一個看起來很權威的數字裡。
# 這不是要你不信任規範——而是要你知道：規範是**模型的輸出**,
# 模型會更新（TEM 三代的演進就是證據），而懂得掀開引擎蓋的
# 人，才有資格參與更新它。
#
# 第二部的工具至此全部到齊：統計地基、短中長期模型、組合、
# 檢驗、危害。最後一章，我們把所有東西帶回家——用台灣的
