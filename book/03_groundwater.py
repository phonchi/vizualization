...
# ## 23.1 一口井如何感受到地震
#
# 含水層是能儲存並讓地下水流動的地層，水分布在孔隙或裂縫中。飽和含水層
# 受到較不透水地層覆蓋、地下水承受壓力時，稱為承壓含水層。壓力水頭可以
# 想像成這股壓力能把連通的水柱托高多少；井水位能否反映它，還要看井與
# 含水層如何連通。
#
...
#
# 同一份資料同時有水位、氣壓與水溫。本圖先並列水位與氣壓，看看水位變動時，
# 氣壓是否也有變化，或兩欄是否一起中斷。水溫留到後面的缺測案例再看。
#
...
# 反應。本圖的水位與氣壓各自使用不同縱軸，不能按曲線高度比較強弱。
#
# %% tags=["remove-input"]
...
# ## 23.9 缺測欄位與仍在工作的感測器
#
# 花蓮井（HWA）在這段教材資料中的水位有部分時段缺測，並非整欄沒有紀錄；
# 水溫也有自己的資料涵蓋範圍。下面先列出各欄在十分鐘平均後的非缺值點數，
# 再畫出水溫，讓我們分別檢查各欄。
#
# 非缺值只表示該數值經過讀取器的缺值旗標轉換後仍有保留，不保證儀器與資料
# 品質正常。缺測原因還需查維運或資料釋出紀錄，也不能由缺測判定那段時間
# 水位沒有反應。
#
# %% tags=["remove-input"]
hwa = gt.read_groundwater(CACHE_DIR / "edu-gw-hualien2024.tgz", "HWA",
                          resample="10min")
print("水位非缺值點數（10 分鐘平均後）：", hwa.water_level_cm.notna().sum())
print("上層水溫非缺值點數（10 分鐘平均後）：", hwa.temp_upper_C.notna().sum())
fig = px.line(hwa.dropna(subset=["temp_upper_C"]), y="temp_upper_C",
              labels={"Time": "時間（UTC）", "temp_upper_C": "上層水溫（°C）"},
              color_discrete_sequence=[PALETTE[1]])
fig.update_traces(line_width=1.5)
apply_layout(fig, title="花蓮（HWA）井：水位部分時段缺測，水溫另行檢視",
             showlegend=False)
fig
