...
本檔統一全站的記號、公式所有權、風格與排版規則。檔名前綴保留舊網址，顯示章號依 `_toc.yml` 的閱讀順序。
撰寫或修改任何一章之前先讀這裡。讀者版記號摘要放在附錄 A。
...
| 累積條件率／網格期望数 | $\Lambda(t)$／$\Lambda_{jk}$ | 補償子通常依賴隨機歷史，不能逐路徑當成期望數；網格期望另依預報條件定義 |
...
- 讀者是有基礎統計的大學生，不預設地震學或點過程背景。每章約15–20分鐘只是參考，不為湊長度增加文字。
- 全書沿「目錄→隨機性→點過程→經驗律→估計→模型→檢驗→台灣物理觀測」推進。章首先承接前文尚未解決的問題，章尾自然帶向下一步。
- 不把每章寫成獨立論文綜述，不套用固定的參數／誤解／研究前沿／結語骨架。必要限制放在相關論述旁，術語先介紹再使用。
- 主文以連貫散文說清動機、直覺與圖例。圖之前有觀察問題、之後有解釋；避免連續公式和表格中斷閱讀。
- 主文保留核心公式、單位與必要條件；可略讀的短補充用 MyST admonition 加 `:class: dropdown`，長推導依主題集中附錄 A–G，提供往返連結。
- 三個基礎章使用明確標示的合成例子，`gdms_toolkit.teaching.learning_catalog` 共用固定seed目錄。真實資料、模擬與示意不得混稱。
- 零習題、零quiz。不要加入設計調參面板、裝飾動畫或與學習無關的操作流程。

全站每個主文及附錄頁末列「參考資料與延伸閱讀」，列齊實際使用來源，不以3–5筆上限省略引用。
由入門到進階排列，至少一項合法免費可讀來源，提供作者／機構、年份、可點閱標題及中文導讀。
教材指出相關章節，論文優先DOI並標示免費版／預印本／訂閱限制。先讀本地原始文獻核實，不只看檔名。
不公開本機PDF，不引用第6節禁止公开的材料。來源頁碼與論點對照留在 `reference/notes/`。
...
- 章首樣板 cell（`# %% tags=["remove-input"]`）一律：
  ...
  setup_plotly()
  ```
- 所有公開頁的程式 cell 加 `tags=["remove-input"]`；無教學價值的操作回顯另加 `remove-output`。原始notebook仍保留程式。繪圖；圖以 Plotly 為主，套 `apply_layout()`，
  用 `ACCENT`／`QUAKE_COLOR`（紅只給事件標記）／`SEQUENTIAL="Blues"`／`PALETTE`。
- **每章圖依教學需要，通常不超過6–8張**；每個 cell 執行 **< 30 秒**；亂數一律 `np.random.default_rng(seed)`。
