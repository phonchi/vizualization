...
# ### 先看預報交出的是什麼
#
# **期望數預報**（expected-count forecast）每箱只交一個平均數。
# **完整分布預報**（full-distribution forecast）則交出零次、一次、兩次等各種結果的機率。
# 相同期望數可以配上不同的變異與尾端。
# 所以，期望數本身不足以重建完整分布。
#
# **適當評分規則**（proper scoring rule）用於完整分布。
# 它讓誠實報出真實分布，在重複結果下有最佳平均分數。
# **一致評分函數**（consistent scoring function）則對準某個摘要，例如期望數。
# 其最佳平均分數出現在報出真實期望數時。
# 兩者都談重複結果的平均，不保證每次觀測的排名。
#
# 本章把期望數放入 Poisson 公式，是兩層選擇。
# 該公式可作比較期望數的一致分數。
# 若再用 Poisson 模擬尾端，就另加了計數分布假設。
# 選對分數，不會自動證實模擬分布也正確。
# 附錄 E 給出兩種用途的推導。
#
...
# - Brehmer 等（2025），[義大利預報的統計評估](https://doi.org/10.1785/0220240209)。[免費作者稿](https://arxiv.org/abs/2405.10712)；§2 分開完整分布的適當評分與期望數的一致評分。
