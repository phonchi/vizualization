...
### 正相鄰差、絕對差與 b-positive

第7–8章的b-positive只保留時間相鄰的正規模差，並可設正的差值門檻。Tinti與Gasperini（2024）第435–436頁則同時比較正差、負差、絕對差及不同配對方式。因此以下三項不能混用：

| 操作 | 使用資料 | 主要條件與限制 |
|---|---|---|
| 正相鄰差 | $d_i=m_i-m_{i-1}>d_c$ | 保留時間方向；差值門檻須預先指定 |
| 絕對相鄰差 | $|m_i-m_{i-1}|>d_c$ | 同時保留升降；對時間變動漏測的反應未必相同 |
| 不重疊配對 | $(m_2-m_1),(m_4-m_3),\ldots$ | 在原規模iid假設下配對獨立，但只使用約一半配對 |

對連續iid指數規模，令 $D=Y-X$，條件於 $D>d_c\ge0$，有

$$P(D-d_c>z\mid D>d_c)=e^{-\beta z},\qquad z\ge0.$$

所以在這個理想邊際分布下，可以從正差的超額平均估 $\beta$。本段不把未修正的連續公式當成離散目錄的精確估計式；取整規模有格距與零差值，須使用相應移位幾何分布。相鄰差也可能共用事件，不能直接將原始差值列視為獨立樣本套標準誤。

b-positive的觀測動機是降低短期餘震不完整的影響；其有效性仍受漏測規則、差值門檻與規模分布隨時間變動的影響。從完整iid模型推導出的正差分布，並不自動證明任意漏測後仍成立。Tinti與Gasperini的比較是限制證據，不是所有情境下選絕對差或正差的統一處方。

本書主線仍使用已交代的離散GR估計。閱讀其他positive系列方法時，先核對選配規則，不能只按方法名稱推斷公式與樣本數。
...
用它估區間時，最優點須適合局部二次近似。
最大化對數概似時，以負 Hessian 作觀測資訊矩陣，再用其反矩陣近似共變異數。
這個資訊矩陣須正定；等價地，內部最大點的對數概似 Hessian 應負定。
若最佳化的是負對數概似，才直接使用其正定 Hessian。
資訊矩陣奇異或非正定時，不能直接套用上述區間近似；對負變異數取絕對值沒有統計依據。
