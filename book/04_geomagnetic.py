...
# ## 21.6 參考站選擇本身也是模型選擇
#
# 接著嘗試換用卑南（TTN）。比較殘差之前，要先確認新城與參考站在相同時刻
# 有多少可計算的配對，再檢查標準差是否為有限值。資料不足時，這次比較就
# 無法成立；即使兩組都能計算，不同的共同時段也可能影響標準差。
#
# %% tags=["remove-input"]
mag_ttn = gt.read_geomagnetic(CACHE_DIR / "edu-mag-ttn.tgz", "TTN",
                              resample="1min")
diff_ttn = (mag["F_calc"] - mag_ttn["F_calc"]).dropna()
for ref_station, residual in [("CSG", diff), ("TTN", diff_ttn)]:
    paired = residual[np.isfinite(residual)]
    n_pairs = len(paired)
    if n_pairs < 2:
        print(f"減 {ref_station}：可計算配對只有 {n_pairs} 筆，資料不足，"
              "無法比較殘差標準差。")
        continue
    residual_std = (paired - paired.mean()).std()
    if not np.isfinite(residual_std):
        print(f"減 {ref_station}：有 {n_pairs} 筆可計算配對，但標準差不是有限值，"
              "無法比較，需再查數值與資料品質。")
        continue
    print(f"減 {ref_station} 後殘差標準差：{residual_std:.2f} nT"
          f"（可計算配對 {n_pairs} 筆）")

# %% [markdown]
# 本書這段資料中，TTN沒有得到可比較的殘差標準差，因此這次不能替兩個
# 參考站排名。應先查共同時段的非缺值資料與品質標記；僅憑這項輸出，還
# 不能確定是哪個環節造成資料不足。
#
# 在資料可比較的條件下，較小的殘差表示差分組合在該資料窗中更平穩，卻
# 不能證明背景已完全去除；目標訊號也可能被共同減掉。若在看過地震時間之後反覆挑選參考站，
...
# 主要反映磁場方向及其緩慢變化。ULF（ultra-low-frequency）指超低頻
# 磁場擾動，研究時仍須指定實際採用的頻帶。文獻中的 ULF 極化指標，會先
# 選定頻帶再比較擾動功率，與這裡直接計算原始場的比值不同。
#
# %% tags=["remove-input"]
zh = (mag.Z / mag.H)
...
# 方式，再檢查每個頻帶的背景和不確定性。
#
# Nyquist 頻率是取樣率的一半，表示取樣對可解析頻率設下的界線。一秒取樣
# 的 Nyquist 頻率是 0.5 Hz；一分鐘取樣則是約 0.0083 Hz。後者仍可能保留
# 更慢的變化，但無法完整分析較高頻率的 ULF 訊號。降取樣前還須適當抑制
# 高頻成分，避免混疊；平均如何改變資訊，見{doc}`附錄 G <appendix_g_observations>`。
#
