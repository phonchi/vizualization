...
本附錄需要機率分布、微分與期望值。對應 {doc}`一致性檢驗 <17_test_space_magnitude>`、{doc}`模型比較 <18_test_comparison>`及 {doc}`模型組合 <19_ensembles>`，各節推導後可回到主文的圖例。

## E.0 同一份預報陣列如何進入檢驗

一個預報窗的資料是 177 格 × 25 個規模箱。
每個元素是期望數，觀測使用相同格箱的整數計數。
時間窗為左閉右開；規模先取整到 0.1，再放入左閉右開的名目規模箱。
第一部的 40 個 91.31 天窗口共覆蓋 3652.4 天；年底剩餘 0.6 天不評分。
目前目錄在這段尾端沒有目標地震。若換版本，必須重新檢查，不能把尾端事件塞進最後一窗。

### 十欄匯出與重新分格

CSEP 文字檔依序列出 `LON_0 LON_1 LAT_0 LAT_1 Z_0 Z_1 MAG_0 MAG_1 RATE FLAG`。
前八欄界定經緯度、深度與規模的上下界，`RATE` 為該窗的格箱期望數，`FLAG` 為啟用旗標。
檔案外另保存時間窗、目錄版本、模型參數及單位。十欄本身不足以指定整個實驗。
本站示範檔含一行欄名；外部讀取器是否接受欄名，須依其格式設定處理。

177 格是投影座標中的方格，不能只把四角反投影後的外接矩形當成相同幾何。
`write_csep_10col` 只展示欄位順序。外接框彼此可能重疊，不能直接拿來評分。
`write_csep_10col_regridded` 才把各格期望數分配到不重疊的 0.1° 經緯度格。

令來源格 $c$ 與輸出格 $g$ 的交集面積比例為 $\widetilde W_{gc}$。
本站在輸出格內用 $5\times5$ 個中點取樣，轉到投影座標後判定來源格。
這是面積近似；為使每個來源格的全部期望數都被分配，再令

$$W_{gc}=\frac{\widetilde W_{gc}}{\sum_h\widetilde W_{hc}},\qquad
\Lambda'_{gk}=\sum_c W_{gc}\Lambda_{ck}.$$

於是 $\sum_g W_{gc}=1$，故每個規模箱都滿足
$\sum_g\Lambda'_{gk}=\sum_c\Lambda_{ck}$。
若某個來源格完全沒有取樣點，程式會報錯；增加取樣密度後才能輸出。
完全不與 R 相交的格省略，邊界格分得部分期望數。
欄正規化消除總量誤差，沒有消除邊界取樣誤差；提高解析度仍需做收斂檢查。
第 16–18 章的評分始終使用原始 177 格，不混用重新分格結果。

### 教學版檢驗的統計量

本書使用完整的 Poisson 計數對數機率，包括同箱多事件的階乘項：

$$\ell=\sum_j[-\Lambda_j+\omega_j\ln\Lambda_j-\ln(\omega_j!)].$$ (eq:jpoll)

約定 $0\ln0=0$；零率箱出現事件時，分數為負無窮。
cL 固定觀測總數，將率正規化後模擬多項計數；L 同時模擬 Poisson 總數。
S 先加總時間與規模，M 先加總時間與空間。
本書聯合 cL／L 保留窗、格、規模，因此也檢查時間分配。
不同軟體若採省略階乘項的點事件分數，模擬排序可能不同，不能僅憑檢驗名稱直接對照數值。

分位數分數是模擬分數不大於觀測值的比例，包含同分。
實作僅容許浮點捨入尺度的誤差，不任意調整統計門檻。
模擬每次只儲存抽中的箱，不配置「模擬數 × 全部格箱」矩陣。
這保留相同的多項抽樣分布，且記憶體不隨兩者乘積成長。
...
### E.2.1 評分對準平均數，還是整個分布？

**完整分布預報**（full-distribution forecast）給出所有計數機率 $P(N=n)$。
**期望數預報**（expected-count forecast）只交出 $\Lambda=E[N]$。
例如兩個分布可有相同平均，卻有不同零次機率及尾端。
比較前先確認模型交付哪一種物件，不能從一個 RATE 欄自行讀出全部分布。

**適當評分規則**（proper scoring rule）讓真實分布在期望意義下得到最佳分數。
本書沿用越大越好的記分方向。若 $Q$ 是真實分布，則適當性要求

$$E_Q[S(Q,N)]\ge E_Q[S(P,N)]$$

對所考慮的分布類成立；只有 $P=Q$ 才取等號時，稱為**嚴格適當**（strictly proper）。
對數分數 $S(P,N)=\ln P(N)$ 的期望差是 $D_{\rm KL}(Q\|P)\ge0$，
須採相應的可積條件與零機率慣例。
這不保證任意一次觀測的排名；它描述長期平均的誘因。
Graham 等（2024）的 pyCSEP 更新文章 §Ranking scores 討論對數與 Brier 分數的不同懲罰。

**一致評分函數**（consistent scoring function）針對平均數等摘要。
若真實均值為 $\mu$，最佳期望分數應在報出 $\Lambda=\mu$ 時取得。
Poisson 形式的分數

$$S_{\rm P}(\Lambda,N)=N\ln\Lambda-\Lambda-\ln N!$$

即具有這項均值性質，不要求真實計數服從 Poisson。
對 $\mu,\Lambda>0$，先相減使只依賴觀測的階乘項消去，再取期望：

$$E[S_{\rm P}(\mu,N)-S_{\rm P}(\Lambda,N)]
=\Lambda-\mu+\mu\ln\frac{\mu}{\Lambda}\ge0.$$

令 $r=\Lambda/\mu$，右式是 $\mu(r-1-\ln r)$；由 $\ln r\le r-1$ 得證。
這個分數差只需要有限均值，避免單獨期待的階乘項可能不可積。
$\mu=0$ 時非負計數幾乎必為零，最佳值為 $\Lambda=0$；正觀測配零預報則受無窮懲罰。
相同均值的不同分布，在這種均值比較下無法區分。
Brehmer 等（2025）§2 的框架正是先分清這兩種預報物件。

| 操作 | 所需主張 | 不會自動得到的結論 |
|---|---|---|
| 比較期望數的平均 Poisson 形式分數 | 分數對均值一致 | 實際計數具有 Poisson 變異 |
| 以 Poisson 模擬 N／L 參考分布 | 指定計數分布及相依結構 | 這些假設已由評分公式驗證 |
| 對完整機率分布用適當規則評分 | 完整分布及所選規則 | 每次結果都會排對名次 |
| 為分數差報區間 | 時間／事件相依的推論條件 | 單憑均值一致性就有正確涵蓋率 |

因此，總數模型不合時仍可比較平均率分數，但原本檢定的尾端或區間未必有效。
反過來，通過某個一致性檢定也不保證有較高相對技巧。
完整分布的評分與期望數比較，可並列服務不同的研究問題。
...
在相同網格與觀測下比較模型 A 與 B，$N>0$ 時，

$$I=\frac{\ell_A-\ell_B}{N}
=\frac1N\sum_i\ln\frac{\Lambda_A(j_i)}{\Lambda_B(j_i)}
-\frac{\Lambda_A^{\rm tot}-\Lambda_B^{\rm tot}}N.$$ (eq:igpe)

若先正規化率形狀 $p_A(j)=\Lambda_A(j)/\Lambda_A^{\rm tot}$，則

$$I_c=\frac1N\sum_i\ln\frac{p_A(j_i)}{p_B(j_i)}.$$ (eq:igpec)

前者包括總量差，後者只比較條件形狀。重複事件仍逐顆配對，但不能因此視為獨立。
零事件資料不定義每事件增益；直接比較整份對數概似差即可。
...
AICc 型修正採 $\mathrm{AICc}=-2\ln L+2k+2k(k+1)/(N-k-1)$。以模型 B 為基準，另定義

$$I_{\mathrm{AICc}}=\frac{\mathrm{AICc}_B-\mathrm{AICc}_A}{2N}.$$ (eq:igpe-aicc)

Rhoades 等人（2014）的 IGPEc 使用這種參數彈性修正。它與 {eq}`eq:igpec` 的條件形狀增益 $I_c$ 不同：後者正規化總率，沒有扣參數懲罰。分母要求兩模型皆滿足 $N>k+1$；AICc 修正有其模型與樣本假設，不是所有相依點過程的通用修正。
...
**基準模型**（baseline model）提供原始格箱率 $\Lambda_{1,jk}$。
**輔助模型**（conjugate model）提供另一張非負空間資訊 $z_{i,j}$，不一定是完整率預報。
Rhoades 等（2014）式 2–3 先以**保序轉換**（order-preserving transformation）形成乘子：

$$\Lambda_{H,jk}=\Lambda_{1,jk}\exp\left[a+\sum_{i=2}^I f_i(z_{i,j})\right],$$

$$f_i(z)=b_i[\ln(1+z)]^{c_i},\qquad b_i\ge0,\quad c_i>0.$$

此處明定自然對數；換對數底時必須連同 $b_i$ 換算。
原文將輔助預報先沿規模箱加總，再轉換其空間排序。
若輸入是應變等其他帶單位資料，須先指定單位或無因次化尺度，再談轉換參數。
$f_i$ 不反轉原有大小次序，但可改變差距；參數仍需從學習資料估計。
因乘子為正，基準的零率支撐也仍為零；輔助資訊不會自動救回那些格箱。

$a$ 控制整體尺度。令未正規化總數為
$Z=\sum_{jk}\Lambda_{1,jk}\exp[\sum_i f_i(z_{i,j})]$，若要固定總數為 $N_*>0$，就取

$$a=\ln(N_*/Z),\qquad Z>0.$$

若總數本身也待估，則不能事後聲稱它必須保持基準總數。
使用非條件 Poisson 概似並自由估 $a$ 時，對 $a$ 微分得到 $N_{\rm learn}-e^a Z$；
在正學習事件數下，最優整體期望數等於學習事件數。
這是該擬合設定的結果，不是所有乘法混合的守恆律。

用三個格的合成例子即可看到差別。
基準期望數為 $(2,3,5)$，輔助值為 $(0,1,2)$。
選示意轉換 $f(z)=\ln(1+z)$，得到乘子 $(1,2,3)$。
相乘後變成 $(2,6,15)$，總數從 10 變成 23。
若要保持總數 10，乘上 $10/23$，得到

$$\Lambda_H=\left(\frac{20}{23},\frac{60}{23},\frac{150}{23}\right),\qquad
\sum_j\Lambda_{H,j}=10.$$

這是代數示例，不是義大利新模型或觀測成績。
它區分「利用輔助排序改形狀」與「決定總量」兩個步驟。
本節後面的 KL 表達描述成品分布間的資訊差，不能代替上述建構。
...
- Graham 等（2024），[New Features in the pyCSEP Toolkit for Earthquake Forecast Development and Evaluation](https://doi.org/10.1785/0220240197)。§Ranking scores 比較適當評分規則與 Brier 分數，先辨認預報格式與分數的方向。
