...
下列說明不需要先登入資料服務或執行下載。可先回到
{doc}`臺灣觀測總覽 <01_overview>`，查看各類資料的物理意義。
...
回到{doc}`資料產品與品質 <02_download>`，繼續閱讀觀測資料的取得與解讀。
...
回到{doc}`資料產品與品質 <02_download>`，對照時間與缺值的處理。
...
回到{doc}`地下水位觀測 <03_groundwater>`，對照頻譜與氣壓響應的例子。
...
回到{doc}`波形與地震目錄 <05_seismic>`，繼續比較原始訊號和目錄。

## G.5 RINEX、座標轉換與地表位移

RINEX 依版本定義標頭、觀測量與曆元格式。以教材中的 RINEX 2 檔名
`hual0930.24o.gz` 為例，它包含站碼、年積日與年份等資訊；年積日必須
按該年是否閏年解讀；本例為 2024-04-02。此檔頭明列 GPS 時間、
30 秒間隔；曆元解析須保留秒數，不能把每分鐘的 00 與 30 秒合併。
標頭中的 `APPROX POSITION XYZ` 是地心直角
近似座標，不是整天各曆元的精密位置。
...
回到{doc}`GNSS 地表位移 <06_gnss>`，對照原始觀測與座標產品。
...
回到{doc}`臺灣展望 <23_taiwan_outlook>`，繼續閱讀大埔案例的機率與次數比較。
花蓮多觀測短窗則見{doc}`花蓮案例 <07_case_hualien2024>`。
